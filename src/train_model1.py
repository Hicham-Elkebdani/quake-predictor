...
# ASSUMPTION: colonnes 'time', 'latitude', 'longitude', 'magnitude', 'depth_km'
...
df = df.dropna(subset=['time','latitude','longitude','magnitude']).reset_index(drop=True)
...
# --- SOLUTION POUR GÉRER L'INFINI ---
# 1. Identifier la valeur maximale finie pour cette colonne
# Le choix d'une valeur de remplacement est crucial.
# Une bonne pratique est d'utiliser une valeur très élevée (mais finie),
# ou la valeur maximale observée + 1.
max_finite_time = Xf.loc[np.isfinite(Xf['time_since_last_event_days']), 'time_since_last_event_days'].max()
# Si max_finite_time est NaN (parce que tout est inf), utilisez LOOKBACK_DAYS comme max.
replacement_value = max_finite_time if pd.notna(max_finite_time) else LOOKBACK_DAYS

# 2. Remplacer les infinis par cette valeur (ou une autre constante)
# On utilise replacement_value + 1 pour s'assurer que l'infini est représenté
# par la valeur la plus grande après mise à l'échelle.
Xf['time_since_last_event_days'] = Xf['time_since_last_event_days'].replace(
    [np.inf, -np.inf], 
    replacement_value + 1
)
...
model = XGBClassifier(
    ...
    learning_rate=0.05,
    # === AJOUTER CETTE LIGNE ===
    base_score=0.5, # Force le base_score à 0.5 (ou une autre petite valeur > 0)
    # ===========================
